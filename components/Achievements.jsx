...
const getIcon = (iconType) => {
    switch (iconType) {
        ...
        case 'users': return <Users size={28} className="text-green-400" />;
            ...
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
                <FadeIn>
                    <div className="text-center mb-16">
                        ...
                        <p className="mt-4 text-slate-400">Competitions, Leadership & Professional Memberships</p>
                    </div>
                </FadeIn>
                    ...
                    {portfolioData.achievements.map((item, idx) => (
                        <FadeIn key={idx} delay={idx * 100} className="h-full">
                            <div className="relative group p-1 rounded-2xl bg-gradient-to-b from-white/10 to-transparent hover:from-purple-500/30 transition-all duration-500 h-full">
                                <div className="relative p-6 rounded-xl bg-slate-900/80 backdrop-blur-xl border border-white/5 h-full flex flex-col items-start hover:bg-slate-900/60 transition-colors">
                                    <div className="flex items-start gap-4 w-full">
                                        <div className="w-14 h-14 rounded-full bg-slate-800/50 border border-white/10 flex items-center justify-center flex-shrink-0 group-hover:scale-110 group-hover:bg-slate-800 transition-all shadow-lg shadow-purple-900/10">
                                            {getIcon(item.iconType)}
                                        </div>
                                        <div className="flex-1">
                                            <h3 className="text-lg font-bold text-white mb-1 group-hover:text-purple-300 transition-colors">{item.title}</h3>
